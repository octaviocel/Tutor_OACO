	...
	<div class="container-fluid">
		<div class="row">
			...
			<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
				...
				<div class="row">
					<div class="col-10">
						<form action="<%=request.getContextPath()%>/Usuario/guardar"
							method="post">
							<input type="hidden" name="idguardar"
								value="${UsuarioActualizar.getId()}" />
							<div class="mb-3 row">
								<label for="correo" class="col-sm-2 col-form-label">Correo</label>
								<div class="col-sm-10">
									<input type="email" name="correo" id="correo"
										value="${UsuarioActualizar.getCorreo()}" class="form-control"
										id="staticEmail">
								</div>
							</div>
							<div class="mb-3 row">
								...
								<div class="col-sm-10">
									<input type="password" name="password" class="form-control"
										value="${UsuarioActualizar.getPassword()}" id="inputPassword">
								</div>
							</div>
							...
							<div class="mb-3 row">
								...
								<div class="col-sm-10">
									<select name="persona"
										value="${UsuarioActualizar.getPersona().getId()}">

										<c:forEach var="plat" items="${listaPersonas}">
											<option name="personaOption" value=" ${plat.getId()} ">${plat.getNombre()}
												${plat.getApePaterno()}</option>
										</c:forEach>
							...
							<div class="mb-3 row">
								...
								<div class="col-sm-10">
									<select name="rol"
										value="${UsuarioActualizar.getRol().getId()}">

										<c:forEach var="rol" items="${listaRoles}">
											<option name="rolOption" value=" ${rol.getId()} ">${rol.getNombres()}</option>
										</c:forEach>
							...
							<div class="mb-3 form-check">
								<input type="checkbox" class="form-check-input" name="status"
									value="${UsuarioActualizar.getStatus()}" id="status"> <label
									class="form-check-label" for="status">Estatus</label>
							</div>
							...
							<div class="mb-3 row">
								...
								<div class="col-sm-4">
									<input type="date" name="fechaVigencia" class="form-control"
										id="fechaVigencia"
										value="${UsuarioActualizar.getFechaVigencia()}">
								</div>
							</div>
							<div class="row">
								<div class="col-2">
